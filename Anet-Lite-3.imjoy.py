...
from anet.data.file_loader import FileLoader
...
class ImJoyPlugin():
    ...
    async def train_test(self, my=None, json_path=None):
        ...
        if json_path is None:
            ...
            # json_path = "/atlas/home/yhu/ImJoyWorkspace/default/Anet-Lite/datasets/anet_png/config.json"
    ...
    def get_opt(self, config, work_dir):
        ...
        for key in [input_key for input_key in opt.channel.keys() if input_key in inputs]:
            opt.input_channels.append(
                (opt.channel[key]["name"], {'filter': "*" + opt.channel[key]["filter"] + "*", 'loader': ImageLoader(scale=0.5)},))

        for out in outputs:
            print("add target_channel:", out.get("name"))
            opt.target_channels.append((out.get("name"), {'filter': "*" + out.get("name") + "*", 'loader': ImageLoader(scale=0.5)},))
