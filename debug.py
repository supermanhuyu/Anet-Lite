...
class ImJoyPlugin():
    ...
    async def auto_train(self, configPath):
        conf_path = self.fs_readfile(configPath["configPath"])

        async def start_train(err, data=None):
            if err:
                print(err)
                return

            # conf_path = "datasets/example/config.json"
            mask_config = {
                "channel": {
            ...
            self.get_data_by_config(config=config_json)
            self.get_mask_by_json(config=config_json)

            self._opt = my_opt(config_json, mask_config)
    ...
    def get_data_by_config(self, config):
        samples = config["samples"]
        for sample in samples:
            sample_annotation = sample["annotation"]
            sample_data = sample["data"]
            self.fs_readfile(sample_annotation)
            for file in sample_data:
                self.fs_readfile(file)
        pass
    ...
    def get_mask_by_json(self, config):
        ...
        for sample in samples:
            ...
            generate_mask(files_proc=[sample_annotation])
            # sample_data = sample["data"]
            # for file in sample_data:
            #     print(file.replace("/tmp", "datasets"))
        return True
        pass
    ...
    def fs_readfile(self, path):
        ...
        if api.fs.readFile(path, 'utf8', cb):
            return saved_path
